...
\item Considerando los datos de la pregunta 1, calcule la probabilidad de que la varianza muestral sea cuando menos 2.77. Utilice la tabla adecuada de las que se presentan a continuación (7 o 8).
...
\item Los datos que a continuación se dan son las edades de 19 estudiantes de una determinada universidad: 19, 20, 21, 21, 24, 21, 20, 25, 17, 23, 28, 29, 27, 25, 25, 28, 20, 25, 31; con una desviación estándar muestral de 3.8472. Las edad de los estudiantes de dicha universidad es una variable aleatoria normal. 

\begin{enumerate}
    \item Estudie si la media de la edades de los estudiantes es mayor a 16 años. Utilice una confianza del 95\%. Especifique, la variable de estudio, la media poblacional, la confianza, la significancia, el tipo de intervalo (añadiendo si la la varianza es conocida o desconocida), la probabilidad del intervalo, fórmula del intervalo, datos del enunciado y tabla utilizada (9 o 10).
    
    \item Estudie si la media de la edades de los estudiantes es cuando mucho 22 años. Utilice una confianza del 97\%. Especifique, la variable de estudio, la media poblacional, la confianza, la significancia, el tipo de intervalo, la probabilidad del intervalo, fórmula del intervalo, datos del enunciado y tabla utilizada (9 o 10).
    
    \begin{minipage}[t]{0.45\textwidth}
\centering
<<echo = F, message = F>>=
library(kableExtra)
p = seq(0.95,0.99, by = 0.01)
q = round(qt(p, df = 18),4)
...
%     Escriba la función de masa de probabilidad. Calcule $E(X)$
    ...
    \item El Servicio de Impuesto Internos (SII), en el 2022, anuncia que en promedio la retención poblacional histórica de dinero en boletas de honorarios emitidas ha sido de 104201 pesos. La desviación estándar poblacional histórica que se considera conocida es de 22600. ¿Cuál es el intervalo de 96 por ciento de confianza para la estimación de la media poblacional histórica de las retenciones de las boletas de honorarios emitidas, si en un muestra de 630 boletas emitidas la media muestral es de 99126 pesos?, interprete Especifique, la variable de estudio, la media poblacional, la confianza, la significancia, el tipo de intervalo, la probabilidad del intervalo, fórmula del intervalo, datos del enunciado y tabla utilizada (11 o 12).
...
library(kableExtra)
p = seq(0.95,0.99, by = 0.01)
q = round(qt(p, df = 629),4)
df = data.frame("Probabilidad" = p, "Cuantil" = q)
df = as.matrix(df)
@
<<echo=FALSE>>=
kable(df, booktabs = TRUE, align = "c", linesep = "", escape = F, col.names = colnames(df), format = "latex",
          caption = "Distribución t-student, df = 629") |>
    kable_styling(latex_options = c("striped","HOLD_position"))
@
\end{minipage}
\begin{minipage}[t]{0.45\textwidth}
...
Determine el valor de $k$, y calcule la $Var(X)$.
